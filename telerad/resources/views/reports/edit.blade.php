...
@section('orthanc-content')
    <div class="flex justify-between items-center mb-6">
        ...
        <div class="space-x-2">
            ...
            <a href="{{ route('reports.show', $report->id) }}" class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 active:bg-blue-900 focus:outline-none focus:border-blue-900 focus:ring ring-blue-300 disabled:opacity-25 transition">
                View Report
            </a>
        </div>
    </div>
        ...
        <div class="border-t border-gray-200 p-4">
            <form action="{{ route('reports.update', $report->id) }}" method="POST" class="space-y-6">
                ...
                <div class="mb-4">
                    ...
                    <div class="flex mb-2">
                        @if($report->study->modalities && is_array($report->study->modalities) && count($report->study->modalities) > 0)
                            @php 
                                $primaryModality = $report->study->modalities[0];
                            @endphp
                    ...
                    <textarea id="findings" name="findings" rows="8" class="w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('findings', $report->findings) }}</textarea>
                </div>
                
                <div class="mb-4">
                    <label for="impression" class="block text-sm font-medium text-gray-700 mb-1">Impression</label>
                    <textarea id="impression" name="impression" rows="4" class="w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('impression', $report->impression) }}</textarea>
                </div>
                
                <div class="mb-4">
                    <label for="recommendations" class="block text-sm font-medium text-gray-700 mb-1">Recommendations</label>
                    <textarea id="recommendations" name="recommendations" rows="3" class="w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('recommendations', $report->recommendations) }}</textarea>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select id="status" name="status" class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                        ...
                        @elseif($report->status === 'amended')
                            <option value="amended" selected>Amended</option>
                        @endif
                    </select>
                    <p class="mt-1 text-sm text-gray-500">
                        @if($report->status === 'draft')
                            Changing to "Final" will finalize this report and set the finalized date.
                        @elseif($report->status === 'final')
                            This report is finalized. You can only create an amendment now.
                        @elseif($report->status === 'amended')
                            This report has been amended after finalization.
                        @endif
                    </p>
                </div>
                
                <div class="flex justify-end space-x-3">
                    <a href="{{ $referrer ?? route('studies.show', $report->study->orthancId) }}" class="inline-flex items-center px-4 py-2 bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest hover:bg-gray-300 focus:outline-none focus:ring ring-gray-300 disabled:opacity-25 transition">
                        Cancel
                    </a>
    ...
    <script>
        ...
        // Insert the selected template into the appropriate field
        function insertSavedTemplate(fieldId) {
            const select = document.getElementById(fieldId + 'TemplateSelect');
            if (select.value) {
