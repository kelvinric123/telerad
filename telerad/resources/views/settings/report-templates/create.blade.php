...
<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-semibold">Create Report Template</h1>
                ...
                <form action="{{ route('report-templates.store') }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <select id="modality" name="modality" class="w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                            ...
                            @foreach($modalityMap as $code => $name)
                                <option value="{{ $code }}" {{ old('modality', $preselectedModality ?? '') === $code ? 'selected' : '' }}>{{ $code }}: {{ $name }}</option>
                            @endforeach
                        </select>
                        @error('modality')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    ...
                    <div class="mb-4">
                        ...
                        <select id="section" name="section" class="w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                            <option value="findings" {{ old('section') === 'findings' ? 'selected' : '' }}>Findings</option>
                            <option value="impression" {{ old('section') === 'impression' ? 'selected' : '' }}>Impression</option>
                            <option value="recommendations" {{ old('section') === 'recommendations' ? 'selected' : '' }}>Recommendations</option>
                        </select>
                        @error('section')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="mb-4">
                        ...
                        <textarea id="content" name="content" rows="12" required
                            class="w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('content') }}</textarea>
                        @error('content')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
